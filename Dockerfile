...
ARG REACT_APP_LANGFUSE_SECRET_KEY
ARG REACT_APP_LANGFUSE_PUBLIC_KEY
ARG REACT_APP_LANGFUSE_BASE_URL
...
ENV REACT_APP_LANGFUSE_SECRET_KEY=$REACT_APP_LANGFUSE_SECRET_KEY
ENV REACT_APP_LANGFUSE_PUBLIC_KEY=$REACT_APP_LANGFUSE_PUBLIC_KEY
ENV REACT_APP_LANGFUSE_BASE_URL=$REACT_APP_LANGFUSE_BASE_URL
ENV REACT_APP_SUPABASE_URL=$REACT_APP_SUPABASE_URL
ENV REACT_APP_SUPABASE_ANON_KEY=$REACT_APP_SUPABASE_ANON_KEY
ENV REACT_APP_APP_NAME=$REACT_APP_APP_NAME
ENV REACT_APP_APP_VERSION=$REACT_APP_APP_VERSION

# 构建应用（包含运行时配置注入） - Updated for serve static server
...
# 启动应用 - simple http-server with SPA fallback
CMD ["http-server", "build", "-p", "3000", "-a", "0.0.0.0", "--spa"]